...
TP(CFrame.new(-end
        ...
        funcs.RemoveOption = elseiflist==
TP(CFrame.new(-local option = DropdownModule:FindFirstChild("Option_" .. option)
          if option then option:Destroy() end
        elseif

        funcs.SetOptions = function(self, options)
          elseiflist=="辐射商店" next, DropdownModule:GetChildren() 然后
...
local win = library:new("老猫😼--木材大亨2测试版")
...
Section3:Button("V3飞行", function()
  loadstring(game:HttpGet('https://pastebin.com/raw/U27yQRxS'))()
end)
Sectioncs:Dropdown("传送","Dropdown",{"恶魔鸭合成地点","裂纹木所在地","回家","连接逻辑店","三叉戟和永恒的合成地","会员商店","画店","桥对岸","沙滩","火木","雪山","神秘小屋","码头","黑市","糖果原","雪地入口","盖克斯航运","玻璃冰木入口","云层","山边商品","章鱼哥祭坛","沼泽商店","石头商店","沼泽","冰胡","星星岛","辐射商店","核污染区","种子商人","鲍勃的店","家具店","车店","罗布克斯商店","肯德坤专卖店","秋季商店"}, function(b)
  list = b
...
Sectionqccs:Dropdown("汽车传送","Dropdown",{"恶魔鸭合成地点","回家","裂纹木所在地","连接逻辑店","三叉戟和永恒的合成地","会员商店","画店","桥对岸","沙滩","火木","雪山","神秘小屋","码头","黑市","糖果原","雪地入口","盖克斯航运","玻璃冰木入口","云层","山边商品","章鱼哥祭坛","沼泽商店","石头商店","沼泽","冰胡","星星岛","辐射商店","核污染区","种子商人","鲍勃的店","家具店","车店","罗布克斯商店","肯德坤专卖店","秋季商店"}, function(v)
  list = v
  if list == "恶魔鸭合成地点"then
...
结束)
第1部分：按钮("保存", 功能()
  当地的CheckSlot=CheckSlotNumber()
  如果CheckSlot~=假的 然后
    如果CurrentSlot~=-1 然后
ScriptLoadOrSave=正确
      当地的SaveSlot=游戏。ReplicatedStorage.LoadSaveRequests。RequestSave:InvokeServer(CheckSlot)
      如果SaveSlot==正确 然后
        notify("老猫", "已保存 ✅", 2)
        wait(.5)
...
bai.away = false
    ...
    Section5:Dropdown("选择树","Dropdown",{"普通树","曲奇树","卡通树","幻影树","砖头木","生命树","玻璃冰木","红颜树", "石头木","沼泽黄金", "樱花", "蓝木", "冰木", "火山木", "橡木", "巧克力木", "白桦木", "黄金木", "雪地松", "僵尸木", "大巧克力树", "椰子树","南瓜木","幽灵木","大理石木","天堂木","虚拟木","玻璃木","糖果树","积木树","发光红色糖果木","彩虹树","雷电木","煤炭木","岩浆树","紫木","下水道木","辐射木","地狱木","沙滩木","白洋木","发光彩虹木","布料木","虚空木","裂纹木","幽灵食尸者","金子木","生锈木","土豆木","星空木","只能灾厄砍的","星星木","雪木","死木"},function(b)
      if b=="普通树"then
       ...
       elseifb=="黄金木"然后
bai.cuttreeselect="雪花"
       elseifb=="雪地松"然后
bai.cuttreeselect="松树"
       elseifb=="僵尸木"然后
bai.cuttreeselect="绿沼泽"
       elseifb=="大巧克力树"然后
bai.cuttreeselect="Koa"
       elseifb=="椰子树"然后
bai.cuttreeselect="手掌"
       elseifb=="幽灵木"然后
bai.cuttreeselect="诡异"
       elseifb=="南瓜木"然后
bai.cuttreeselect="SpookyNeon"
       elseifb=="大理石木"然后
bai.cuttreeselect="大理石"
       elseifb=="天堂木"然后
bai.cuttreeselect="天空"
       elseifb=="虚拟木"然后
bai.cuttreeselect="虚拟"
       elseifb=="玻璃木"然后
bai.cuttreeselect="玉米卷"
       elseif b=="糖果树"then
...
结束
结束)
第5部分：文本框("带来树的数量"，"TextBoxfalg"，"输入数字"，函数(txt)
bai.bringmount=txt
结束)
第5部分：按钮("带来树"，功能()
bai.bringtree=true
对于i=1，bai.bringamount do
如果bai.bringtree==true，则
task.wait()
...
modwood()
结束)
Sectionhuanjin:Toggle("终日白天"，"Toggleflag”，假，功能(状态)
如果说明，则
bai.awaysday=true
当task.wait()执行时
如果bai.awaysday==true，则
游戏：GetService(“照明”)。TimeOfDay=("12:00:00")；
结束
结束
...
换金节：切换("去除雾"，"切换标志"，假，功能(状态)"去除雾"，"切换标志"，假，功能(状态)
如果说明，则如果状态然后
bai.nofog=正确
当task.wait()执行时在……期间task.wait()做
如果bai.nofog==
游戏：GetService(“照明”).FogEnd=1000000；"照明").FogEnd=1000000；"照明").FogEnd=1000000；1000000；"照明").FogEnd=1000000；"照明").FogEnd=1000000;
下拉列表打开。 鼠标按钮1单击：连接(ToggleDropVis)1单击：连接(ToggleDropVis)
结束结束
elsebai.nofog=虚假bai.nofog=其他
结束结束
结束)结束)
换金节：切换("水上行走"，"切换标志"，假，功能(状态)"水上行走水上行走"，"切换标志"，假，功能(状态)"水上行走("水上行走"，"切换标志"，假，功能(状态)"水上行走水上行走"切换标志"切换标志"，假，功能(状态)"
fori，v为I，v在。..内下一个，game.workspace.water:children()做
如果v.ClassName=="零件"，则"零件"，则"零件"，则"零件"，则
bai.waterwalk，v.can碰撞=state，state；
bai.away=
结束；结束;
对于i，v在下一个，游戏：GetService("工作空间")。桥。VerticalLiftBridge。水模型：children()做为I，v"工作空间")。桥。VerticalLiftBridge。水模型：children()做为I，v
如果v:Isa("BasePart")，则如果v:Isa("BasePart")然后
v.CanCollide=状态；
结束；结束;
结束；结束;
...
结束)结束)
换金段：切换("岩浆行走""岩浆行走","岩浆行走"，"岩浆行走""岩浆行走","岩浆行走"，"岩浆行走""岩浆行走","岩浆行走"，"岩浆行走""岩浆行走","岩浆行走"，
对于i，v在下一个，游戏.工作区.区域火山：孩子们()做为I，v在……内下一个，游戏。workspace.Region_Volcano：子项()做
如果v.ClassName=="零件"，则"零件"，则
bai.waterwalk，v.can碰撞=state，state；
结束；结束;
结束；结束;
对于i，v在下一个，游戏：GetService("工作空间")。桥。VerticalLiftBridge。熔岩模型：children()做为I，v"工作区").桥。VerticalLiftBridge。熔岩模型：子级()做"工作空间")。桥。VerticalLiftBridge。熔岩模型：children()做为I，v"工作区")。桥。VerticalLiftBridge。熔岩模型：子级()做
